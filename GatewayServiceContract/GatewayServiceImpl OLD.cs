...
namespace GatewayServiceContract
{
	public class GatewayServiceImpl : IGatewayService 
	{
		...
		public OperationResult RegisterDevice(string deviceId, Device device)
		{
			try
			{
                //Ensure that the id is the right ine
                device.Id = deviceId;
		...
		public OperationResult RegisterSensors(string deviceId, Sensor[] sensorList)
		{
			try
			{
                //Ensure the device id
                foreach (Sensor sensor in sensorList) sensor.DeviceId = deviceId;
