...
namespace DeviceServer.Base.Cndep
{
    ...
    public class CndepMessageRequest
    {
        ...
        public CndepMessageRequest(byte[] messageBytes)
        {
            ...
            {
                throw new ArgumentException(StringUtility.Format("Malformed message: invalid length: {0}", messageBytes.Length), "messageBytes");
            }
            ...
            int dataLength = messageBytes.Length - MinimalRequestLength;
            if (dataLength > 0)
            {
    ...
    public class CndepMessageResponse
    {
        ...
        public CndepMessageResponse(byte[] messageBytes)
        {
            ...
            int dataLength = messageBytes.Length - MinimalResponseLength;
            if (dataLength > 0)
            {
                Data = new byte[dataLength];
                Array.Copy(messageBytes, IndData, Data, 0, dataLength);
