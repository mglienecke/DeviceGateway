...
{
	public class PythonCallback : AbstractCallback
	{
					...
					if (runtime == null)
					{
						switch (CallbackImplementation)
						{
							case CallbackImplementation.Python:
						        ...
						        options["Debug"] = true;
								runtime = Python.CreateRuntime(options);
								break;
							default:
		...
		{
			get { return(Runtime.GetEngine(PythonLanguage)); }
		}
		...
		protected Func<object, object> PythonCallbackFunc { get; set; }
		...
		protected override bool InitializeExecutionEnvironment()
		{
			try
			{
				...
				{
					PythonCallbackFunc = myCallback;
				}
				else
				{
					throw new InvalidOperationException(string.Format(Properties.Resources.ErrorPythonCallbackIsNotDefined, callbackName));
				}
				IsInitialized = true;
		...
		public override CallbackResultData ExecuteCallback(CallbackPassInData data)
		{
			...
			try
			{
				dynamic scriptResult = PythonCallbackFunc(data);
				result = (scriptResult != null) && (scriptResult is CallbackResultData) ? (scriptResult as CallbackResultData) : null;
				return (result);
			}
			catch (System.Exception ex)
			{
				log.ErrorFormat(Properties.Resources.ErrorCallingPythonCallback, PythonCallbackFunc.Method.ToString(), ex.ToString());
				throw;
			}
		}
		...
		protected PythonCallback(string symbolicName, CallbackType callbackKind)
		{
			SymbolicName = symbolicName;
			CallbackType = callbackKind;
			CallbackImplementation = CallbackImplementation.Python;
		}
		...
		public PythonCallback(string symbolicName, CallbackType callbackKind, string executionExpression)
			: this(symbolicName, callbackKind)
		{
			...
			{
				throw new InvalidOperationException(Properties.Resources.ErrorCreatingPythonCallback);
			}
		}
		...
		public PythonCallback(string symbolicName, CallbackType callbackKind, string executionExpression, string externalFileName)
			: this(symbolicName, callbackKind)
		{
			// always ignored
