...
namespace fastJSON
{
    ...
    public class JSON
    {
        ...
        public object ToObject(string json, Type type)
        {
            ...
            {
                ArrayList ht = new JsonParser(json).Decode() as ArrayList;
                if (ht == null) return null;
                return ParseArray(ht, null, type);
            }
            else
            {
        ...
        public object ToObject(string json, Dictionary<string, object> globalTypes, Type type)
        {
            if (type.IsArray || type is IList)
            {
                ArrayList ht = new JsonParser(json).Decode() as ArrayList;
                if (ht == null) return null;
                return ParseArray(ht, globalTypes, type);
            }
            else
            {
                Dictionary<string, object> ht = new JsonParser(json).Decode() as Dictionary<string, object>;
                if (ht == null) return null;
                return ParseDictionary(ht, globalTypes, type);
            }
        }
        ...
        private object ChangeType(object value, Type conversionType)
        {
            if (conversionType == typeof(int))
                ...
                return CreateDouble((string)value);
        ...
        private object ParseArray(IList list, Dictionary<string, object> globaltypes, Type type)
        {
            if (type.IsArray)
            {
                Array array = Array.CreateInstance(type.GetElementType(), list.Count);
                for (int i = 0; i < list.Count; i++)
                {
                    array.SetValue(ToUserType(list[i], globaltypes, type.GetElementType()), i);
                }
                ...
                if (type is IList)
                {
                    ...
                    {
                        result = (IList)typeof(List<>).MakeGenericType(type.GetGenericArguments()[0]).GetConstructor(null).Invoke(null);
                        for (int i = 0; i < list.Count; i++)
                        {
                            result.Add(ToUserType(list[i], globaltypes, type.GetGenericArguments()[0]));
                        }
                    }
                    else
                    {
                        ...
                        for (int i = 0; i < list.Count; i++)
                        {
                            result.Add(ToUserType(list[i], globaltypes, type.GetElementType()));
                        }
                    }
        ...
        public object ToUserType(object value, Dictionary<string, object> globaltypes, Type type)
        {
            if (type.IsArray || type is IList)
            {
                return ParseArray((IList)value, null, type);
            }
            else
            {
                return ParseDictionary((Dictionary<string, object>)value, null, type);
            }
        }
...
#else
                        else if (pi.changeType == typeof(ArrayList))
                            oset = CreateArrayList((ArrayList)v, pi.pt, typeof(object), globaltypes);
        ...
        private object CreateArrayList(ArrayList data, Type pt, Type bt, Dictionary<string, object> globalTypes)
        {
            ArrayList col = new ArrayList();
            // create an array of objects
            foreach (object ob in data)
